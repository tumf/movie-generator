#!/bin/bash
# VOICEVOX Core installation script for Linux
set -e
...
echo "🎙️  VOICEVOX Core Installation for Linux"
echo "=================================================="
echo ""
...
if [ "$ARCH" != "x86_64" ]; then
	...
	echo "This script only supports x86_64 (amd64)"
	exit 1
fi
...
WHEEL_URL="https://github.com/VOICEVOX/voicevox_core/releases/download/${VOICEVOX_VERSION}/voicevox_core-${VOICEVOX_VERSION}-cp310-abi3-manylinux_2_34_x86_64.whl"
...
DOWNLOADER_URL="https://github.com/VOICEVOX/voicevox_core/releases/download/${VOICEVOX_VERSION}/download-linux-x64"
...
export VOICEVOX_ONNXRUNTIME_PATH="$VOICEVOX_DIR/onnxruntime/lib/libvoicevox_onnxruntime.so.1.17.3"
EOF

echo "✓ Environment configuration created at: $CONFIG_FILE"

# Step 5: Update config/default.yaml (optional)
echo ""
...
if [ -f "$CONFIG_YAML" ]; then
	...
	if grep -q "^audio:" "$CONFIG_YAML"; then
		...
		echo "   Please add manually if needed"
	fi
else
...
if [ $? -eq 0 ]; then
	...
	echo ""
	echo "Next steps:"
	echo "  1. Add to your shell profile (~/.bashrc):"
